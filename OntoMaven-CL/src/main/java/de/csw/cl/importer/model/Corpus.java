 ...
 * shrink after import directives have been resolved and replaced by the actual
...
public class Corpus {
    ...
    // The documents should be valid with respect to the XCL2 schema, after
    // XInclude directives have been resolved.
    // The state of the Corpus is include-inconsistent if there are circular
    // XInclude directives.
    // Titlings should not contain XInclude directives in the Ontomaven
    // namespace.
    private final HashMap<Document, File> documents = new HashMap<Document, File>();

    // All top-level titlings (by name) that are accessible for importation.
    // During the importation process, new titlings, which become accessible
    // because their surrounding titling has been imported, might be added to
    // this map.
    private HashMap<String, Element> accessibleTitlings = new HashMap<String, Element>();

    // TODO: maintain a hashmap of accessible importation elements and their
    // include IRI
    // private HashMap<Element, String> accessibleImportations = new
    ...
    // During the import resolution process, new entries may be added.
    public Includes includes;

    // Contains mappings between the IRIs of XInclude directives and the
    // relative path for the cached copy.
    // All Xinclude directives in the XInclude closure of documents that
    // reference remote objects
    // should have mappings in the catalog that map to relative file paths.
    // The catalog should not contain mappings in the Ontomaven domain that are
    // not used by some XInclude directive.
    ...
    /**
     * @param corpusDir
     *            the directory containing XCL2 documents and optionally the
     *            catalog.xml file and /includes directory
    ...
    public Corpus(File corpusDir) throws ConflictingTitlingException,
            MissingCatalogEntryException, MissingIncludeEntryException {
        // load the includes subdirectory, if it exists, into an Includes object
        includes = new Includes(corpusDir);
        // load the XML catalog, if it exists, into an XMLCatalog object
        catalog = new XMLCatalog(new File(corpusDir, "catalog.xml"));

        if (!(includes.verifySequentialFileNames())) {
            // TODO: do something if the includes are not named as sequential
            // numbers
            System.out
                    .println("Warning: include files are not sequentially numbered.");
        }
        // filter the corpus files ending in .xcl (case insensitive) into an
        // File array
        ...
        // load the corpus files and process them
        if (!(docFiles == null)) {
        ...
        // TODO: verify that each file in includes is used in some XInclude
        // directive.
     ...
     * @throws MissingCatalogEntryException
     */
    public void addDocument(Document doc, File documentFile)
            throws ConflictingTitlingException, MissingCatalogEntryException,
            MissingIncludeEntryException {
    ...
    public XMLCatalog cleanCatalog() throws MissingCatalogEntryException,
            MissingIncludeEntryException {
        ...
        for (Entry<String, String> entry : catalog.getMappings().entrySet()) {
            ...
            if (name.startsWith(XMLUtil.NS_ONTOMAVEN.getURI().toString())) {
                if (!(verifyInclude(name))) {
                    LOG.warn("Warning: An unused catalog entries has been removed: "
                            + name);
                    System.out
                            .println("Warning: An unused catalog entries has been removed: "
                                    + name);
    ...
    /**
     * Returns a {@link Boolean} indicating if an IRI is used for XInclude in
     * the corpus. This implementation only checks for an exact String match of
     * the IRI. TODO: also check for equivalent IRIs?
     * 
     * @param uri
     *            the IRI to be checked.
     * @return true if there is an XInclude that references uri in the XInclude
     *         closure of the corpus documents.
     * @throws MissingIncludeEntryException
     * @throws MissingCatalogEntryException
     */
    private boolean verifyInclude(String uri)
            throws MissingCatalogEntryException, MissingIncludeEntryException {
    ...
    /**
     * Returns a {@link Boolean} indicating if an IRI is used for XInclude in an
     * {@link Element}.
     * 
     * @param e
     ...
     * @throws MissingCatalogEntryException
     */
    private boolean verifyIncludeLoop(Element e, String uri)
            throws MissingCatalogEntryException, MissingIncludeEntryException {
        String ename = e.getName();
        // System.out.println("Checking: " + ename);
        if (e.getNamespace().equals(XMLUtil.NS_XINCLUDE)) {
            if (ename.equals("include")) {
                if (e.getAttributeValue("href").equals(uri))
                    return true;
                if (verifyIncludeLoop(followInclude(e), uri))
                    return true;
            }
            return false;
        }
        if (!(e.getNamespace().equals(XMLUtil.NS_XCL2)))
            return false;
        if (ename.equals("Restrict") || ename.equals("Construct")) {
            for (Element c : e.getChildren()) {
                if (verifyIncludeLoop(c, uri))
                    return true;
            }
        }
        return false;
    ...
    /**
     * Returns an {@link Iterable} over all documents contained in this
     * {@link Corpus}.
     * 
     * @return an {@link Iterable} over all documents contained in this
     *         {@link Corpus}.
     */
    ...
    public Element getImportableTitling(String name) {
        return accessibleTitlings.get(name);
    }
    ...
    private void addImportableTitling(Element e)
        ...
        Element existingImportableTitling = accessibleTitlings.get(titlingName);
        if (existingImportableTitling != null) {
            ...
            accessibleTitlings.put(titlingName, e.clone());
        }
    }

    public Element followInclude(Element e)
            throws MissingCatalogEntryException, MissingIncludeEntryException {
        if (isXInclude(e)) {
            ...
            try {
                ...
                try {
                    ...
                    System.out
                            .println("Warning: no entry for this key in includes. Not following.");
    ...
    public void write(File resultDir) throws FolderCreationException {
        if (size() > 0) {
            ...
            if (!(unresolvedImports.isEmpty())) {
                ...
                System.out
                        .println("Warning. There are unresolved importations:");
    ...
    private File getOutputFile(File resultDir, Document document) {
        return new File(resultDir, getOriginalFile(document).getName()
                .replaceAll("myText", "resultText"));
    }
    ...
    private LinkedList<String> getUnresolvedImports() {
        ...
        for (Document document : documents) {
            ...
            XMLUtil.performRecursivelAction(document.getRootElement(),
                    new XMLUtil.Action() {
                        public void doAction(Element e) {
                            if (e.getName().equals("Import") && isUntitled(e)) {
                                unresolvedImports.add(getName(e) + " in "
                                        + fileName);
                            }
                        }
                    });
    ...
    public static boolean isTitling(Content e) {
        switch (e.getCType()) {
        case Element:
            return (((Element) e).getName().equals("Titling") && ((Element) e)
                    .getNamespace().equals(XMLUtil.NS_XCL2));
        default:
            return false;
        }
    }
    ...
    public static boolean isRestrict(Content e) {
        switch (e.getCType()) {
        case Element:
            return (((Element) e).getName().equals("Restrict") && ((Element) e)
                    .getNamespace().equals(XMLUtil.NS_XCL2));
        default:
            return false;
        }
    ...
    public static boolean isConstruct(Content e) {
        switch (e.getCType()) {
        case Element:
            return (((Element) e).getName().equals("Construct") && ((Element) e)
                    .getNamespace().equals(XMLUtil.NS_XCL2));
        default:
            return false;
        }
    ...
    public static boolean isImport(Content e) {
        switch (e.getCType()) {
        case Element:
            return (((Element) e).getName().equals("Import") && ((Element) e)
                    .getNamespace().equals(XMLUtil.NS_XCL2));
        default:
            return false;
        }
    ...
    public static boolean isXInclude(Content e) {
        switch (e.getCType()) {
        case Element:
            return (((Element) e).getName().equals("include") && ((Element) e)
                    .getNamespace().equals(XMLUtil.NS_XINCLUDE));
        default:
            return false;
        }
    }
